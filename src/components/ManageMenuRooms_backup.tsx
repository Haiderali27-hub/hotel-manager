...
const ManageMenuRooms: React.FC<ManageMenuRoomsProps> = ({ onBack }) => {
  ...
  const handleAddMenuItem = async () => {
    ...
    // Validation
    if (!menuName.trim()) {
      showFeedback('Please enter menu item name', true);
      return;
    }

    if (!menuPrice.trim()) {
      showFeedback('Please enter menu item price', true);
      return;
    }

    const priceValue = parseFloat(menuPrice);
    if (isNaN(priceValue) || priceValue <= 0) {
      showFeedback('Please enter a valid price greater than 0', true);
      return;
    }
    ...
    try {
      ...
      showFeedback(`Menu item "${menuName.trim()}" added successfully!`);
      loadData();
    } catch (err) {
      showFeedback('Failed to add menu item. Please try again.', true);
      console.error(err);
    } finally {
  ...
  const handleUpdateMenuItem = async () => {
    ...
    if (!editingMenuItem) {
      showFeedback('No menu item selected for editing', true);
      return;
    }
    ...
    try {
      ...
      setShowMenuForm(false);
      showFeedback(`Menu item "${menuName.trim()}" updated successfully!`);
      loadData();
    } catch (err) {
      showFeedback('Failed to update menu item. Please try again.', true);
      console.error(err);
    } finally {
      setIsSubmitting(false);
    }
  const handleDeleteMenuItem = async (id: number) => {
    if (confirm('Are you sure you want to delete this menu item?')) {
      ...
      try {
        ...
        showFeedback('Failed to delete menu item. Please try again.', true);
        console.error(err);
      } finally {
        setIsSubmitting(false);
      }
  ...
  const handleAddRoom = async () => {
    ...
    // Validation
    if (!roomNumber.trim()) {
      showFeedback('Please enter room number', true);
      return;
    }

    // Check if room number already exists
    if (rooms.some(room => room.number === roomNumber.trim())) {
      showFeedback(`Room ${roomNumber.trim()} already exists. Please choose a different room number.`, true);
      return;
    }

    if (!roomPrice.trim()) {
      showFeedback('Please enter room price', true);
      return;
    }

    const priceValue = parseFloat(roomPrice);
    if (isNaN(priceValue) || priceValue <= 0) {
      showFeedback('Please enter a valid price greater than 0', true);
      return;
    }
    ...
    if (!finalRoomType) {
      showFeedback('Please specify room type', true);
      return;
    }
    ...
    try {
      ...
      showFeedback(`Room ${roomNumber.trim()} (${finalRoomType}) added successfully!`);
      loadData();
    } catch (err) {
      showFeedback('Failed to add room. Please try again.', true);
      console.error(err);
    } finally {
      setIsSubmitting(false);
    }
  ...
  const handleDeleteRoom = async (id: number) => {
    ...
    const roomName = roomToDelete ? `Room ${roomToDelete.number}` : 'this room';
    
    if (confirm(`Are you sure you want to delete ${roomName}?`)) {
      setIsSubmitting(true);
      try {
        await deleteRoom(id);
        showFeedback(`${roomName} deleted successfully!`);
        loadData();
      } catch (err) {
        showFeedback('Failed to delete room. Please try again.', true);
        console.error(err);
      } finally {
        setIsSubmitting(false);
      }
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: '1rem', justifyContent: 'flex-end' }}>
              ...
              >
                {isSubmitting ? 'Processing...' : (editingMenuItem ? 'Update' : 'Add')}
              </button>
            </div>
          </div>
        </div>
